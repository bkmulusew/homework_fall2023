...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # TODO: update the PG actor/policy network once using the advantages -- Done
        info: dict = self.actor.update(obs, actions, advantages)

        # step 4: if needed, use all datapoints (s_t, a_t, q_t) to update the PG critic/baseline
        if self.critic is not None:
            # TODO: perform `self.baseline_gradient_steps` updates to the critic/baseline network -- Done
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            discount_fn = self._discounted_return
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # TODO: use the helper function self._discounted_reward_to_go to calculate the Q-values -- Done
            discount_fn = self._discounted_reward_to_go

        for trajectory_rewards in rewards:
            q_vals_for_traj = discount_fn(trajectory_rewards)
            q_values.append(q_vals_for_traj)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        return np.full_like(rewards, total_discounted_return, dtype=np.float32)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in range(len(rewards)):
            timesteps = np.arange(len(rewards) - t)
            discounts = self.gamma ** timesteps
            discounted_rewards[t] = np.sum(discounts * rewards[t:])
